...
// Fragment shader adapted from user-provided design
const FRAG = `
#ifdef GL_ES
...
export const LaserFlow = ({
  ...
  // Start fully visible to avoid any refresh-time alpha artifacts
  ...
  useEffect(() => {
    ...
    try {
      renderer = new THREE.WebGLRenderer({
        antialias: false,
        // Enable true alpha for reliable transparency across navigations
        alpha: true,
        depth: false,
        stencil: false,
      ...
      console.error('LaserFlow: WebGL init failed, using CSS fallback.', e);
      mount.classList.add('laser-fallback');
      return () => {
        mount.classList.remove('laser-fallback');
      };
    }
    ...
    // Use transparent clear so the laser overlays the page content nicely
    ...
    const material = new THREE.RawShaderMaterial({
      ...
      fragmentShader: FRAG,
      uniforms,
      // Enable transparency and additive blending so the beam "glows" on dark backgrounds
      transparent: true,
      depthTest: false,
      depthWrite: false,
    ...
    const setSizeNow = () => {
      // Robust sizing: try client sizes, then DOMRect, then window fallback
      let w = mount.clientWidth || 0;
      let h = mount.clientHeight || 0;
      if (w < 2 || h < 2) {
        const r = mount.getBoundingClientRect();
        w = Math.max(w, Math.floor(r.width));
        h = Math.max(h, Math.floor(r.height));
      }
      if (w < 2 || h < 2) {
        w = Math.max(w, window.innerWidth || 1);
        const parentH = (mount.parentElement as HTMLElement | null)?.clientHeight || 0;
        h = Math.max(h, parentH || window.innerHeight || 1);
      }
    ...
    setSizeNow();
    // Failsafe: some layouts report 0 size on first tick; force re-measure
    requestAnimationFrame(setSizeNow);
    setTimeout(setSizeNow, 200);
    const ro = new ResizeObserver(scheduleResize);
    ro.observe(mount);
    // Extra safety: also listen on window-level resize/orientation/load
    window.addEventListener('resize', scheduleResize, { passive: true });
    window.addEventListener('orientationchange', scheduleResize as any, { passive: true } as any);
    ...
    const animate = () => {
      ...
      // Only pause when the document is hidden. IntersectionObserver can
      // transiently report non-intersection on refresh; don't gate on it.
      ...
      // No fade-in to ensure immediate visibility after refresh
      ...
      // Keep DPR stable for consistent visual alignment; disable adaptive changes
    ...
    return () => {
      ...
      ro.disconnect();
      window.removeEventListener('resize', scheduleResize as any);
      window.removeEventListener('orientationchange', scheduleResize as any);
      window.removeEventListener('load', scheduleResize as any);
      io.disconnect();
      ...
      uniformsRef.current = null as any;
    };
